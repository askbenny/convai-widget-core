...
const VARIANT_CLASSES = {
  ...
  ghost:
    "text-base-primary border border-transparent bg-transparent hover:bg-base-hover active:bg-base-active",
};
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  return (
    ...
    >
      {icon && (
        <Icon
          className={clsx("transition-[margin] duration-200", iconOnly && "-mx-0.5", iconClassName)}
          name={icon}
        />
      )}
      <SizeTransition visible={!!children} dep={children}>
        <span className="block whitespace-nowrap max-w-64 truncate px-1.5">{children}</span>
      </SizeTransition>
    </button>
  );
});
