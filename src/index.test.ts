...
import { describe, it, beforeAll, beforeEach, afterEach, expect, afterAll, vi } from "vitest";
...
describe("askbenny-convai", () => {
  ...
  it.each(Variants)("$0 variant should go through a happy path", async (variant) => {
  ...
  it.each(Variants)("$0 expandable variant should go through a happy path", async (variant) => {
    setupWebComponent({
      "agent-id": "basic",
      transcript: "true",
      "text-input": "true",
    ...
    await expect.element(page.getByText("User transcript")).toBeInTheDocument();
    ...
    await expect.element(page.getByText("You ended the conversation")).toBeInTheDocument();
    ...
    await userEvent.keyboard("{Enter}");
    await expect.element(page.getByText("New text message")).toBeInTheDocument();

    // Established connection
    await expect.element(page.getByText("Chatting with AI agent")).toBeInTheDocument();
  });
  ...
  it.each(Variants)(
    ...
    async (variant) => {
      ...
      await expect.element(page.getByText("Another agent response")).toBeInTheDocument();

      // Agent closed the connection
      await expect.element(page.getByText("The agent ended the conversation")).toBeInTheDocument();
      await expect.element(page.getByText("ID")).toBeInTheDocument();
    }
  );
  ...
  it.each(Variants)(
    ...
    async (variant) => {
      ...
      await expect.element(page.getByText("Goodbye! Have a great day!")).toBeInTheDocument();
  ...
  it.each(Variants)("$0 expandable variant should handle errors", async (variant) => {
    setupWebComponent({
      "agent-id": "fail",
      transcript: "true",
      "text-input": "true",
  ...
  describe("markdown rendering", () => {
    ...
    it("should render lists", async () => {
      ...
      await expect.element(page.getByText("Ordered item 1")).toBeInTheDocument();
      await expect.element(page.getByText("Ordered item 2")).toBeInTheDocument();
    });
    ...
    it("should render blockquotes", async () => {
      await expect.element(page.getByText("Blockquote text")).toBeInTheDocument();
    });
  ...
  describe("markdown link allowlist", () => {
    it("should allow widget domain by default when config omits allowlist", async () => {
      ...
      await expect.element(page.getByRole("link", { name: "Relative link" })).toBeInTheDocument();
    });
    ...
    it("should deny links when markdown_link_allowed_hosts is empty", async () => {
      ...
      await expect.element(page.getByText("No links should be clickable:")).toBeInTheDocument();
      await expect.element(page.getByText("Link text")).toBeInTheDocument();
      await expect.element(page.getByRole("link", { name: "Link text" })).not.toBeInTheDocument();
    });
    ...
    it("should allow only whitelisted domains", async () => {
      ...
      await expect.element(allowedHttps).toHaveAttribute("href", "https://example.com/allowed");

      const allowedHttp = page.getByRole("link", { name: "Allowed http link" });
      await expect.element(allowedHttp).toBeInTheDocument();
      await expect.element(allowedHttp).toHaveAttribute("href", "http://example.com/http-allowed");
  ...
  describe("dismissable behavior", () => {
    it("should not show dismiss button by default (opt-in)", async () => {
      ...
      await expect.element(page.getByRole("button", { name: "Start a call" })).toBeVisible();
      await expect.element(page.getByRole("button", { name: "Dismiss" })).not.toBeInTheDocument();
    });

    it.each(Variants)("$0 variant should allow dismiss and restore via orb", async (variant) => {
      setupWebComponent({
        "agent-id": "basic",
        variant,
        dismissible: "true",
    ...
    it("should dismiss expandable widget completely", async () => {
      ...
      await expect.element(page.getByRole("button", { name: "Open chat" })).toBeVisible();
    });
  });
  ...
  describe("agent status", () => {
    it("should render inline tool status when show-agent-status is enabled", async () => {
      ...
      await expect.element(page.getByText("Tool completed successfully")).toBeInTheDocument();
      await expect.element(page.getByText("Completed", { exact: true })).toBeInTheDocument();
      await expect.element(page.getByText("Working...")).not.toBeInTheDocument();
    });
