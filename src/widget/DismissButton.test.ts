...
describe("Dismiss Button", () => {
  ...
  describe("when dismissible is enabled", () => {
    ...
    it.each(Variants)("$0 variant should restore widget when clicking orb", async (variant) => {
      setupWebComponent({
        "agent-id": "basic",
        variant,
        dismissible: "true",
  ...
  describe("when dismissible is disabled", () => {
    it.each(Variants)("$0 variant should not show dismiss button or orb", async (variant) => {
      setupWebComponent({
        "agent-id": "basic",
        variant,
        dismissible: "false",
      ...
      await expect.element(page.getByRole("button", { name: "Open chat" })).not.toBeInTheDocument();
    });
  });

  describe("when dismissible is not specified (opt-in behavior)", () => {
    it.each(Variants)("$0 variant should not show dismiss button by default", async (variant) => {
      setupWebComponent({
        "agent-id": "basic",
        variant,
      });
