...
describe("buildDisplayTranscript", () => {
  ...
  describe("empty message filtering", () => {
    it.each<{
    ...
    }>([
      {
        ...
        input: [msg("agent", "", { eventId: 2 }), msg("agent", "Done", { eventId: 2 })],
  ...
  describe("grouping by eventId + role", () => {
    it.each<{
      ...
      {
        description: "preserves distinct agent messages with the same eventId",
        input: [msg("agent", "partial", { eventId: 2 }), msg("agent", "full", { eventId: 2 })],
        expected: [{ message: "partial" }, { message: "full" }],
      },
      {
        description: "does not group messages with different eventIds",
        input: [msg("agent", "first", { eventId: 1 }), msg("agent", "second", { eventId: 2 })],
  ...
  describe("tool status", () => {
    it.each<{
      ...
      },
      {
        ...
        input: [msg("agent", "Done", { eventId: 2 }), toolReq(2), toolRes(2, { isError: true })],
    ...
    it("does not attach status when showAgentStatus is false", () => {
      const input = [msg("agent", "Done", { eventId: 2 }), toolReq(2), toolRes(2)];
  ...
  describe("transcript filtering", () => {
    it.each<{
      ...
      {
        description: "filters non-text messages when transcriptEnabled is false",
        input: [msg("agent", "voice", { isText: false }), msg("user", "text", { isText: true })],
  ...
  describe("real HAR flows", () => {
    it.each<{
      ...
      },
      {
        ...
        expected: [{ message: "run all" }, { message: "All done", toolStatus: "loading" }],
      },
    ])("$description", ({ input, expected }) => {
      const result = build(input, { showAgentStatus: true });
      expect(result).toHaveLength(expected.length);
